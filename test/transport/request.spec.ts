...
const me = { statusCode: 200, body: { object: 'me', hosts: { central: 'https://central', regional: 'https://regional' } } };

describe('braultRequest', () => {
	beforeEach(() => clearHostsCache());

	it('routes regional calls to the regional host and returns the flat object whole', async () => {
		const c = ctx([me, { statusCode: 200, body: { object: 'library', id: 'lib_1', name: 'Brand' } }]);
		await expect(braultRequest(c, { plane: 'regional', method: 'GET', path: '/v1/libraries/lib_1' })).resolves.toEqual({
			object: 'library',
			id: 'lib_1',
			name: 'Brand',
		});
	...
	it('returns a flat single object exactly as the API sent it', async () => {
		const c = ctx([me, { statusCode: 200, body: { object: 'file', id: 'f1' } }]);
		await expect(braultRequest(c, { plane: 'regional', method: 'GET', path: '/v1/files/f1' })).resolves.toEqual({ object: 'file', id: 'f1' });
	});

	it('still unwraps a legacy data envelope', async () => {
	...
	it('adds Idempotency-Key on creating POSTs', async () => {
		const c = ctx([me, { statusCode: 201, body: { object: 'library', id: 'x' } }]);
	...
	it('retries once on 429 with a short Retry-After', async () => {
		const c = ctx([
			me,
			{ statusCode: 429, headers: { 'retry-after': '0' }, body: { object: 'error', code: 'rate_limited', message: 'slow down', status: 429 } },
			{ statusCode: 200, body: { object: 'file', id: 'f1' } },
		]);
		await expect(braultRequest(c, { plane: 'regional', method: 'GET', path: '/v1/files' })).resolves.toEqual({ object: 'file', id: 'f1' });
	});

	it('throws NodeApiError with code and request id', async () => {
		const c = ctx([me, { statusCode: 404, body: { object: 'error', code: 'not_found', message: 'No such file', status: 404, request_id: 'req_9' } }]);
	...
	it('does not retry a 429 whose Retry-After exceeds the 10s cap', async () => {
		const c = ctx([
			me,
			{ statusCode: 429, headers: { 'retry-after': '30' }, body: { object: 'error', code: 'rate_limited', message: 'slow down', status: 429 } },
		]);
	...
	it('does not retry a 429 with no Retry-After header at all', async () => {
		const c = ctx([me, { statusCode: 429, body: { object: 'error', code: 'rate_limited', message: 'slow down', status: 429 } }]);
		await expect(braultRequest(c, { plane: 'regional', method: 'GET', path: '/v1/files' })).rejects.toMatchObject({ httpCode: '429' });
		expect((c as never as { calls: Call[] }).calls.length).toBe(2);
	});

	it('retries exactly once even if the retry itself comes back 429', async () => {
		const rateLimited = { statusCode: 429, headers: { 'retry-after': '0' }, body: { object: 'error', code: 'rate_limited', message: 'slow down', status: 429 } };
